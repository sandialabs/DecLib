...
from DecLib import VolumeFormRecon, DiamondVForm_MLP, LieDerivativeM, InteriorProductMLP
from DecLib import LieDerivativeVForm, DiamondVForm_V, InteriorProductV
...
V_lie_derivative = LieDerivativeVForm(meshes)
...
MLP_interior_product = InteriorProductMLP(meshes)
MLP_lie_derivative = LieDerivativeVForm(meshes)
...
velocityflux =  KForm(meshes.dim-1, meshes.dtopo, meshes.dRBundle, 'uflux', create_petsc=True)
...
MLP_interior_product.apply(velocity, velocityflux)
dens_recon.apply(dens1, dens1recon, velocityflux)
MLP_diamond.apply(dens1recon, B1, Mrhs)
MLP_lie_derivative.apply(dens1recon, velocityflux, dens1rhs)
...
M_recon.apply(M, mrecon, velocityflux)
m_lie_derivative.apply(mrecon, velocity, velocityflux, Mrhs)
Mrhs.petsc_vec.pointwiseMult(Mrhs.petsc_vec, velocity.petsc_vec)
energy_change = Mrhs.petsc_vec.sum()
print('mlp lie derivative m change', energy_change)

#check full mlp
dens_recon.apply(dens2, dens2recon, velocityflux)
m_lie_derivative.apply(mrecon, velocity, velocityflux, Mrhs)
MLP_diamond.apply(dens1recon, B1, Mrhs, mode=ADD_MODE)
MLP_diamond.apply(dens2recon, B2, Mrhs, mode=ADD_MODE)
MLP_lie_derivative.apply(dens1recon, velocityflux, dens1rhs)
MLP_lie_derivative.apply(dens2recon, velocityflux, dens2rhs)
Mrhs.petsc_vec.pointwiseMult(Mrhs.petsc_vec, velocity.petsc_vec)
dens1rhs.petsc_vec.pointwiseMult(dens1rhs.petsc_vec, B1.petsc_vec)
...
print('v advected quantity change', energy_change)
...
print('v full change', energy_change)
